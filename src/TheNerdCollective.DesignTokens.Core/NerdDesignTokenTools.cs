...
public static class NerdDesignTokenTools
{
    ...
    public static void WriteCss(NerdDesignTokenOptions options, string path)
    {
        ...
        File.WriteAllText(path, NerdCoreCssGenerator.Generate(options));
    }

    /// <summary>Writes framework-neutral CSS variables only (alias for <see cref="NerdCoreCssGenerator.Generate"/>).</summary>
    public static string ExportCoreCss(NerdDesignTokenOptions options) =>
        NerdCoreCssGenerator.Generate(options);
        ...
        IReadOnlyDictionary<string, string>? typographyRoles = null)
    {
        ...
        if (options.Spacing.Count > 0)
        {
            ...
            builder.AppendLine();
            foreach (var pair in options.Spacing.OrderBy(pair => pair.Key, StringComparer.Ordinal))
            {
                builder.AppendLine($"- **{pair.Key}**: `{pair.Value}`");
            }
        }
        ...
        AppendLayoutCompositions(builder, options);
        AppendOpacityTokens(builder, options);
    ...
    private static void AppendLayoutCompositions(System.Text.StringBuilder builder, NerdDesignTokenOptions options)
    {
        var compositions = NerdLayoutCompositionCatalog.Describe(options);
        ...
        builder.AppendLine();
        builder.AppendLine("## Layout compositions");
        builder.AppendLine();
        foreach (var block in compositions)
        {
            builder.AppendLine($"### {block.Title}");
            builder.AppendLine();
            builder.AppendLine(block.Description);
            builder.AppendLine();
            builder.AppendLine($"- Recipe class: `.{options.Prefix}-recipe-{block.RecipeName}`");
            if (!string.IsNullOrWhiteSpace(block.OverlayClass))
            {
                builder.AppendLine($"- Overlay: `.{block.OverlayClass}`");
            }
    ...
    private static void AppendOpacityTokens(System.Text.StringBuilder builder, NerdDesignTokenOptions options)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("## Opacity overlays");
        builder.AppendLine();
        foreach (var pair in options.Opacities.OrderBy(pair => pair.Key, StringComparer.Ordinal))
        {
            builder.AppendLine(
                $"- **{pair.Key}**: `{pair.Value.BaseToken}` @ {pair.Value.Opacity:0.##} → `.{options.Prefix}-opacity-{pair.Key}`");
        }
    }
}
