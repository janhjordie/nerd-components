...
/// <summary>Shared parsing primitives for built-in and static <see cref="SigNozResponseParser"/>.</summary>
internal static class SigNozResponseParserInternals
{
    public static ObservabilityTimeSeriesResult ParseTimeSeriesCore(
        string json,
    ...
    public static IReadOnlyList<ObservabilityServiceInfo> ParseServicesCore(string json)
    {
        var root = JsonNode.Parse(json) as JsonObject;
        if (root is null)
        {
    ...
    private static void AppendV5Aggregations(JsonNode? aggregationsNode, List<ObservabilityTimeSeriesPoint> points)
    {
        ...
        {
            if (aggregation is JsonObject aggregationObject)
            {
    ...
    private static void AppendSeriesPoints(JsonNode? seriesNode, List<ObservabilityTimeSeriesPoint> points)
    {
        ...
        {
            if (series is JsonObject seriesObject)
            {
    ...
    private static bool TryReadPoint(JsonNode? timestampNode, JsonNode? valueNode, out ObservabilityTimeSeriesPoint point)
    {
        ...
        if (!TryReadTimestamp(timestampNode, out var timestamp) || !TryReadDouble(valueNode, out var value))
        {
            return false;
        }
