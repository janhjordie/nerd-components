...
`/nerd-theme` (konfigurerbart via `ThemeCatalogRoute`) viser den præcise mapning
fra token-aliases til MudBlazor `MudTheme`-props (`Palette.*`, layout, shadows, z-index)
med farver, CSS-variabler og resolved color tokens.
